...
function [AllFCROI, InddAll]=Extract_Feature_kFold(Index,Allsub,Cov, CovName, CovSiteIdx, ROIName, ROIMaskdir, SPMdir, datadir, writedir)
...
for j=1:length(unique(TrainSiteIdx))
    ...
    a=a';
for i=a
fils1(i) = dir(fullfile(datadir{ROI},strcat(TrainSub{i},ROIName{ROI}))); %change the suffix to match with your data
end
fils1=fils1';
filsAll{j}=fils1(a,1);
clear fils1 a
...
InddF=cell(0); AllFCA=[];
for Contrast=1:2
[data,~,head]=rest_readfile(strcat(SPMdir,strcat('/','spmT_000',num2str(Contrast), '.nii')));%Load negative associations
A=[]; Index1=[]; b=[]; 
data1=data(:);
Ind1=find(data1~=0);
